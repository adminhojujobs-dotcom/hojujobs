...
begin
  ...
  return query
    select
      ...
      count(*) filter (where uce.event_name = 'job_card_clicked')::bigint,
      count(*) filter (where uce.event_name = 'rental_card_clicked')::bigint,
      count(*) filter (where uce.event_name = 'sale_card_clicked')::bigint,
      count(*) filter (where uce.event_name = 'deal_outbound_clicked')::bigint,
      count(*) filter (where uce.event_name = 'map_clicked')::bigint,
      count(*) filter (where uce.event_name = 'news_article_clicked')::bigint,
      count(*) filter (where uce.event_name = 'navigation_clicked')::bigint,
      count(*) filter (where uce.event_name = 'post_cta_clicked')::bigint,
      count(*) filter (where uce.event_name = 'my_posts_clicked')::bigint,
      count(*) filter (where uce.event_name = 'admin_clicked')::bigint,
      count(*) filter (where uce.event_name = 'promote_cta_clicked')::bigint,
      count(*) filter (where uce.event_name = any(array['auth_login_clicked','auth_signup_clicked','auth_google_clicked','auth_mode_toggled']))::bigint,
      count(*) filter (where uce.event_name = 'sales_filter_changed')::bigint,
      min(uce.created_at),
      max(uce.created_at)
